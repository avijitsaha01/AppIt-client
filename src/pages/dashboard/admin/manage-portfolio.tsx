...
export default function ManagePortfolio() {
  ...
  const [form, setForm] = useState({ title: '', slug: '', description: '', techUsed: '', client: '', url: '', impact: '', status: 'published' as 'draft' | 'published' })
  const [file, setFile] = useState<File | null>(null)
  const [preview, setPreview] = useState<string | null>(null)

  const { data: portfolios, isLoading } = useQuery({
    queryKey: ['portfolios'],
    queryFn: () => api.get<{ portfolios: Portfolio[] }>('/portfolios').then(r => r.data.portfolios),
  })

  const createMutation = useMutation({
    mutationFn: (fd: FormData) => api.post('/portfolios', fd),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['portfolios'] }); resetForm() },
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, fd }: { id: string; fd: FormData }) => api.put(`/portfolios/${id}`, fd),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['portfolios'] }); resetForm() },
  })
  ...
  const resetForm = () => {
    ...
    setForm({ title: '', slug: '', description: '', techUsed: '', client: '', url: '', impact: '', status: 'published' })
    setFile(null); setPreview(null)
  }

  const openEdit = (p: Portfolio) => {
    setEditing(p); setForm({ title: p.title, slug: p.slug, description: p.description, techUsed: p.techUsed?.join(', ') || '', client: p.client || '', url: p.url || '', impact: p.impact || '', status: p.status })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    fd.append('title', form.title); fd.append('slug', form.slug); fd.append('description', form.description)
    fd.append('status', form.status)
    if (form.techUsed) form.techUsed.split(',').forEach(t => fd.append('techUsed', t.trim()))
    if (form.client) fd.append('client', form.client)
    if (form.url) fd.append('url', form.url)
    if (form.impact) fd.append('impact', form.impact)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-neutral-900">Manage Portfolio</h1>
          <p className="mt-1 text-[13px] text-neutral-500">Add and manage portfolio projects.</p>
        </div>
      ...
      {!portfolios?.length ? (
        <EmptyState icon={Briefcase} title="No portfolio projects yet" />
      ) : (
        <div className="overflow-hidden rounded-xl border border-neutral-200/70 bg-white">
          <table className="w-full">
            <thead>
              <tr className="border-b border-neutral-100">
                <th className="px-5 py-3.5 text-left text-[11px] font-semibold uppercase tracking-wider text-neutral-500">Project</th>
                <th className="px-5 py-3.5 text-left text-[11px] font-semibold uppercase tracking-wider text-neutral-500">Client</th>
                ...
                <tr key={p._id} className="transition-colors hover:bg-neutral-50">
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      {p.image ? <img src={p.image} alt="" className="h-8 w-8 rounded-md object-cover" /> : <div className="flex h-8 w-8 items-center justify-center rounded-md bg-neutral-100"><Briefcase className="h-4 w-4 text-neutral-500" /></div>}
                      <span className="text-[13px] font-medium text-neutral-900 truncate max-w-[200px]">{p.title}</span>
                    </div>
                  </td>
                  <td className="px-5 py-3.5 text-[13px] text-neutral-600">{p.client || '—'}</td>
                  <td className="px-5 py-3.5">
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex gap-2">
                      ...
                      <button className="text-[12px] font-medium text-red-500 hover:text-red-600" onClick={() => { if (confirm('Delete this project?')) deleteMutation.mutate(p._id) }}>Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => resetForm()}>
          <div className="w-full max-w-2xl rounded-2xl bg-white p-8 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="mb-6 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-neutral-900">{editing ? 'Edit Project' : 'New Project'}</h3>
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-1.5">
                  <Label className="text-[12px] font-medium text-neutral-700">Client</Label>
                  <Input value={form.client} onChange={(e) => setForm(f => ({ ...f, client: e.target.value }))} className="h-9 text-[13px]" />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-[12px] font-medium text-neutral-700">Live URL</Label>
                  <Input value={form.url} onChange={(e) => setForm(f => ({ ...f, url: e.target.value }))} className="h-9 text-[13px]" />
                </div>
              </div>
              <div className="space-y-1.5">
                <Label className="text-[12px] font-medium text-neutral-700">Technologies (comma separated)</Label>
                <Input value={form.techUsed} onChange={(e) => setForm(f => ({ ...f, techUsed: e.target.value }))} className="h-9 text-[13px]" placeholder="e.g. React, Node.js, MongoDB" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-[12px] font-medium text-neutral-700">Business Impact</Label>
                <textarea value={form.impact} onChange={(e) => setForm(f => ({ ...f, impact: e.target.value }))} rows={2} className="w-full rounded-lg border border-neutral-200 bg-white px-3.5 py-2.5 text-[13px] outline-none focus:border-neutral-400" />
              </div>
              <div className="space-y-1.5">
